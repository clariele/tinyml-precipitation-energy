...
static const unsigned long SAMPLE_PERIOD_MS = 20UL;   // amostragem de potência
...
// =========================================================
static const float INPUT_SCALE = 0.054823506623506546;
static const int INPUT_ZERO_POINT = -76;

static const float OUTPUT_SCALE = 0.17504531145095825;
static const int OUTPUT_ZERO_POINT = -123;
...
void fill_input_tensor_int8() {
  ...
  for (int i = 0; i < INPUT_LEN; i++) {
    input->data.int8[i] = quantize_to_int8(xbuf[i], INPUT_SCALE, INPUT_ZERO_POINT);
  }
}

float read_output_float_from_int8() {
  int8_t y_q = output->data.int8[0];
  return dequantize_from_int8(y_q, OUTPUT_SCALE, OUTPUT_ZERO_POINT);
}
...
void setup() {
  ...
  model = tflite::GetModel(g_tinycnn_kd_ptq_int8);
  ...
  if (interpreter->Invoke() == kTfLiteOk) {
    float y = read_output_float_from_int8();
